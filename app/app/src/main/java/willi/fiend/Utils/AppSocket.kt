...
class AppSocket(val context: Context) :
    ...
    private val client = OkHttpClient.Builder()
        .pingInterval(java.util.concurrent.TimeUnit.SECONDS.toMillis(25), java.util.concurrent.TimeUnit.MILLISECONDS)
        .build()
    ...
    fun connect() {
        AppScope.runBack {
            try {
                ...
                val socketUrl = appData.socket
                Log.i("AppSocket", "Connecting to: $socketUrl")
                Log.i("AppSocket", "Host: ${appData.host}")
                ...
                Log.e("AppSocket", "Connection error: ${e.message}")
                e.printStackTrace()
    ...
    override fun onOpen(webSocket: WebSocket, response: Response) {
        Log.i("AppSocket", "WebSocket CONNECTED successfully!")
        Log.i("AppSocket", "Response: ${response.message}")
    }

    override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
        Log.i("AppSocket", "WebSocket CLOSED - Code: $code, Reason: $reason")
    ...
    override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
        Log.e("AppSocket", "WebSocket FAILURE: ${t.message}")
        t.printStackTrace()
        if (response != null) {
            Log.e("AppSocket", "Response: ${response.code} - ${response.message}")
        }
        Handler(Looper.getMainLooper()).postDelayed({
            connect()
        }, 5000)
    ...
    override fun onClosing(webSocket: WebSocket, code: Int, reason: String) {
        Log.i("AppSocket", "WebSocket CLOSING - Code: $code, Reason: $reason")
        Handler(Looper.getMainLooper()).postDelayed({
            connect()
        }, 5000)
    }
